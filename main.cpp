...
#include <imgui.h>
#include <vv_camera.h>
#include <vulkan_visualizer/third_party/vma/vk_mem_alloc.h>
#include <algorithm>
#include <array>
#include <chrono>
#include <cmath>
...
#define VK_CHECK(expr)                                                                        \
    do {                                                                                      \
        ...
        if (_vk_result_ != VK_SUCCESS)                                                        \
            throw std::runtime_error("Vulkan error: " + std::to_string(static_cast<int>(_vk_result_))); \
    } while (false)
#endif
...
private:
    ...
    void build_point_field() {
        ...
        for (int iz = 0; iz < gridRes; ++iz) {
            for (int iy = 0; iy < gridRes; ++iy) {
                for (int ix = 0; ix < gridRes; ++ix) {
                    ...
                    auto color = add_color(mul_color(base, 0.55f + 0.45f * height),
                                           mul_color(warm, 0.25f * densityNorm + 0.5f * emissive));
        ...
        const std::array<vv::float3, 4> guideLights = {
            vv::make_float3(-0.55f, 0.62f, 0.25f),
            vv::make_float3(0.48f, -0.52f, -0.4f),
            vv::make_float3(0.12f, 0.75f, -0.6f),
            vv::make_float3(-0.2f, -0.7f, 0.15f)};
    ...
    void create_uniform_buffer() {
        create_buffer(sizeof(CameraUniforms),
